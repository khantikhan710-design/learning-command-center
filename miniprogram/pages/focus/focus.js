...
Page({
  data:{minutes:25,seconds:0,running:false,label:'设置你的专注时长',today:0,timer:null,minuteOptions:[],secondOptions:[],pickerValue:[25,0],initialSeconds:1500,strict:false,breaksLeft:3,onBreak:false,breakSeconds:0,breakTimer:null,lastUnexpectedLeave:0,breakStartAt:0,lastBreakText:''},
  ...
  onShow(){this.setData({today:wx.getStorageSync('focusMinutes')||0});if(this.data.lastUnexpectedLeave){this.setData({lastUnexpectedLeave:0});wx.showToast({title:'3 次离开机会已用完',icon:'none'})}if(this.data.onBreak)this.endBreak('returned')},
  ...
  finish(){this.stop();const minutes=Math.round(this.data.initialSeconds/60);const total=(wx.getStorageSync('focusMinutes')||0)+minutes;wx.setStorageSync('focusMinutes',total);cloudStore.addFocusSession({minutes,endedAt:Date.now(),strict:this.data.strict}).catch(()=>{});this.setData({today:total,label:'本轮完成'});wx.showToast({title:'完成一次专注！',icon:'success'})}
})
